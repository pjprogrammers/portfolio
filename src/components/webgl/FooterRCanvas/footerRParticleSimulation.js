...
import {
  AMBIENT_FIELD_OPACITY,
  AMBIENT_FORMATION_MAX_PROMINENCE,
  FOOTER_R_DEPTH_SIZE_REF,
  FOOTER_R_FORM_STAGGER,
  FOOTER_R_FORMATION_FRACTION,
...
function buildParticles(rSurfacePoints, shapeAnchors, starPositions) {
  return rSurfacePoints.map((anchor, index) => {
    ...
    const particle = {
      ...
      // En el estado ambiental (no footer), solo una fracción forma la figura;
      // el resto queda en el campo orgánico para bajar la densidad del motivo.
      formsShape: Math.random() < FOOTER_R_FORMATION_FRACTION,
    };
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (inFooterFormation) {
        ...
        if (p.formsShape) {
          ...
          p.y = y;
          p.z = z;
          opacity = fade * prominence;
          visibility = fade * prominence;
          particleScale = 1 - dissolveT;
        } else {
          p.x = p.spawnX;
          p.y = p.spawnY;
          p.z = p.spawnZ;
          opacity = fade * prominence * AMBIENT_FIELD_OPACITY;
          visibility = fade * prominence;
          particleScale = 1 - dissolveT;
        }
      }
