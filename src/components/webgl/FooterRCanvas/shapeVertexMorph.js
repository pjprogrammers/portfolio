...
import { DIAMOND_INDICES, DIAMOND_POSITIONS } from "./diamondGeometry.js";
...
const diamondTriangleCache = buildIndexedTriangleCache(
  DIAMOND_POSITIONS,
  DIAMOND_INDICES,
);
...
export function writeMorphedShapePositions(morph, target = morphedPositions) {
  ...
  for (let i = 0; i < STAR_POSITIONS.length; i++) {
    const diamondIndex = Math.min(i, DIAMOND_POSITIONS.length - 3);
    target[i] =
      (STAR_POSITIONS[i] * starWeight +
        DIAMOND_POSITIONS[diamondIndex] * diamondWeight) *
      scale;
  }
