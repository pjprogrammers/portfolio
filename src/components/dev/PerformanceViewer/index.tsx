...
export function PerformanceViewer() {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      setEnabled((value) => {
        ...
        setCollapsed((collapsedValue) => !collapsedValue);
        return true;
      });
    };
  ...
  return (
    <div
      className="hidden min-[1024px]:block pointer-events-auto fixed right-3 bottom-3 z-9999 w-[min(100vw-1.5rem,22rem)] font-mono text-[11px] leading-relaxed text-white/80"
      aria-live="polite"
      ...
      <div className="overflow-hidden rounded-lg border border-white/10 bg-black/80 shadow-2xl backdrop-blur-md">
        ...
        {!collapsed ? (
          <div className="space-y-1 px-3 pb-3">
            ...
            <MetricRow
              label="RAF real"
              value={`${snapshot.rafFps}`}
              valueClassName={formatFpsClass(snapshot.rafFps)}
            />
            <MetricRow
              label="Frame"
            ...
            <MetricRow
              label="DPR canvas"
              value={
                snapshot.effectiveDpr !== null
                  ? `${snapshot.effectiveDpr}`
                  : "—"
              }
            />
